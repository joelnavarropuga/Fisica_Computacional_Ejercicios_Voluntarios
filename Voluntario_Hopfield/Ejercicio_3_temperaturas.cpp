...
int main()
{
    ...
    vector < double > T = {0.01, 0.05, 0.06, 0.07, 0.08, 0.09, 0.1, 0.15, 0.2};
    
    // Ejecuto el algortimo de Metropolis para una configuración inicial aleatoria
    for (int i = 0; i < T.size(); i++)
    {
        ofstream fichero_neuronas_aleatorias("Configuracion_mu_inicial_neuronas_aleatoria_T_" + to_string(T[i]) + ".txt");
    ...
    for (int i = 0; i < T.size(); i++)
    {
        ofstream fichero_neuronas_deformado("Configuracion_mu_inicial_neuronas_deformada_T_" + to_string(T[i]) + ".txt");
...
void algoritmo_Metropolis(vector < vector < int > > s, vector < vector < vector < vector < double > > > > omega_matriz, int N, mt19937_64& generator, ofstream& fichero_neuronas, double T)
{
    //PASO 0: Creo una configuración inicial de la red de neuronas
