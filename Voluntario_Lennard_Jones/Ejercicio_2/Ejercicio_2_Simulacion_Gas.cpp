...
int main()
{
    ...
    // El módulo de la velocidad se pone a 1 para simplificar el problema

    vector < double > v_x(N), v_y(N);

    for (int i = 0; i < N; i++) 
    {
        v_x[i] = abs(cos(angulos[i]));
        v_y[i] = 0.0;
    }
    ...
    // Genero los dicheros donde voy a guardar las posiciones y velocidades de los átomos
    ...
    ofstream archivo_posiciones("Posiciones_Atomos_v_x.txt");
    ofstream archivo_velocidades("Velocidades_Atomos_v_x.txt");
    ...
    for (int k = 1; k <= M; k++) 
    {
        ...
        // Guado las posiciones y velocidades en los ficheros
