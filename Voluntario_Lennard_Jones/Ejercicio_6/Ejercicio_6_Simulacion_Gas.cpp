...
int main()
{
    ...
    ofstream archivo_posicion_atomo("Posicion_Atomo_Individual_ej_6.txt");
    ...
    // Genero una variable para guardar la posición del átomo 0 sin aplicar las condiciones periódicas en el fichero
    double x_aux = x[0];
    double y_aux = y[0];

    // Escribo la posición del átomo 0 en el fichero
    archivo_posicion_atomo << x_aux << ' ' << y_aux << '\n';
    ...
    for (int k = 1; k <= M; k++) 
    {
        ...
        // Trayectoria real del átomo 0 sin aplicar condiciones periódicas
        x_aux += h * w_x[0];
        y_aux += h * w_y[0];
